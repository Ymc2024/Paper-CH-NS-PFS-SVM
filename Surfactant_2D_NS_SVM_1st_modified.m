function [phi,rho,u,v,p,alpha] = Surfactant_2D_NS_SVM_1st_modified(pde,domain,Nx,Ny,time,option)
...
dir_data = [pde.name 'SVM_1st_modified/data'];
...
for nt = 1:nplot
    ...
    h22 = Mphi.*lap_diff(mu_rho_star)./Mrho;
    ...
    % Step 3
    alpha = 0;
    alpha = fsolve(@(alpha) non_fun_alpha(alpha,u_new1,u_new2,u0,v_new1,v_new2,v0,phi_new1,phi_new2,phi0,rho_new1,rho_new2,rho0,phi_star_new,rho_star_new,tmp1,tmp2,tmpu,tmpv),alpha,opts);
...
function r = non_fun_alpha(alpha,u_new1,u_new2,u0,v_new1,v_new2,v0,phi_new1,phi_new2,phi0,rho_new1,rho_new2,rho0,phi_star_new,rho_star_new,rhs1,rhs2,rhs3,rhs4)
global dt Mphi Mrho upsilon gamma1 gamma2 epsilon S1 S2 eta
    ...
    mu_phi_new = -gamma1 * lap_diff((phi_new1 + alpha.*phi_new2 )) + E_derivative_phi(phi_star_new,rho_star_new)+S1/epsilon^2 *((phi_new1 + alpha.*phi_new2) - phi0) ;
    mu_rho_new = -gamma2 * lap_diff((rho_new1 + alpha.*rho_new2 )) + E_derivative_rho(phi_star_new,rho_star_new)+S2/eta^2 *((rho_new1 + alpha.*rho_new2 ) - rho0) ;
    right = fun_inner(mu_phi_new,Mphi.*lap_diff(mu_phi_new)) + fun_inner(rhs1,mu_phi_new)...
           +fun_inner(mu_rho_new,Mrho.*lap_diff(mu_rho_new)) + fun_inner(rhs2,mu_rho_new)...
           +fun_inner(u_new1+ alpha.*u_new2,upsilon.*lap_diff(u_new1 + alpha.*u_new2)) + fun_inner(rhs3,u_new1 + alpha.*u_new2)...
           +fun_inner(v_new1 + alpha.*v_new2,upsilon.*lap_diff(v_new1 + alpha.*v_new2)) + fun_inner(rhs4,v_new1 + alpha.*v_new2);
    r = left - dt*right;
end
